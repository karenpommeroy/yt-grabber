...
export const execute = async (parameters: MessageHandlerParams) => {
    ...
    try {
        await Promise.race([run(params, options, i18n, onUpdate), abortPromise]);
    } catch (error: any) {
        ...
        if (error instanceof TimeoutError) {
            ...
            if (error.message === "Navigating frame was detached") {
                result.warnings.push({
                    title: i18n.t("exceptionYoutubeResolvePlaylists"),
                    description: i18n.t("exceptionYoutubeResolvePlaylistsText", {error: error.name}),
                });
            } else {
                result.errors.push({
                    title: i18n.t("exceptionYoutubeResolvePlaylists"),
